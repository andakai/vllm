...
def test_minimal_kv_cache_uses_configured_builder(monkeypatch):
    from vllm.v1.core import kv_cache_config_builder

    spec = {"layer": object()}
    minimal_config = SimpleNamespace(num_blocks=5)
    calls: list[tuple[object, object, int]] = []

    def build_minimal(config, cache_spec, blocks):
        calls.append((config, cache_spec, blocks))
        return minimal_config

    monkeypatch.setattr(
        kv_cache_config_builder,
