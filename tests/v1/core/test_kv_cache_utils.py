...
def _glm5_tensor_layout(groups):
    _glm5_builder()
    from vllm.models.glm5next.kv_cache_config import _glm5_next_tensor_layout

    return _glm5_next_tensor_layout(groups)
...
def test_get_kv_cache_config_balanced_mamba_hybrid():
    ...
    assert (
        builder.max_memory_usage_bytes_from_groups(vllm_config, groups)
    ...
    planned_config = builder.build_kv_cache_configs(
        vllm_config, [kv_cache_spec], [available_memory]
    )[0]
    assert planned_config.num_blocks == kv_cache_config.num_blocks
    assert planned_config.kv_cache_tensors == kv_cache_config.kv_cache_tensors

    rank_configs = builder.build_kv_cache_configs(
        vllm_config,
    ...
    try:
        override_config = builder.build_kv_cache_configs(
            vllm_config, [kv_cache_spec], [available_memory]
        )[0]
    finally:
    ...
    with pytest.raises(ValueError, match="To serve at least one request"):
        builder.build_kv_cache_configs(
            vllm_config,
            [kv_cache_spec],
