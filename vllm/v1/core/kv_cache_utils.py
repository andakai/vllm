...
def validate_kv_cache_groups(
    kv_cache_specs: dict[str, KVCacheSpec], groups: list[KVCacheGroupSpec]
) -> None:
    """Validate groups returned by a declarative plan provider."""
    planned_names = [name for group in groups for name in group.layer_names]
    if len(planned_names) != len(set(planned_names)):
        raise ValueError("KV cache group plan assigns a layer more than once")
    if set(planned_names) != set(kv_cache_specs):
        raise ValueError("KV cache group plan must assign every KV cache layer")
    for group in groups:
        if not group.layer_names:
            raise ValueError("KV cache group plan cannot contain an empty group")
...
def _pool_bytes_per_block(
    ...
    pool_plan: KVCachePoolPlan | None = None,
) -> int:
    ...
    """
    return _get_kv_cache_bytes_per_block(kv_cache_groups, pool_plan)
...
def _validate_kv_cache_pool_plan(
    kv_cache_groups: list[KVCacheGroupSpec], plan: KVCachePoolPlan
) -> None:
    layer_groups = {
    ...
    planned_layers = [layer for region in plan for layer in region.layers]
    if len(planned_layers) != len(set(planned_layers)):
        raise ValueError("KV cache pool plan assigns a layer more than once")
    if set(planned_layers) != set(layer_specs):
        raise ValueError("KV cache pool plan must assign every projected layer")
    for region in plan:
        if not region.layers or region.size_bytes <= 0:
            raise ValueError("KV cache pool regions must be non-empty and positive")
        if any(
            layer_specs[layer_name].page_size_bytes != region.size_bytes
            for layer_name in region.layers
        ):
            raise ValueError("KV cache layer page must match its planned pool region")
...
def _get_kv_cache_bytes_per_block(
    ...
    if pool_plan is not None:
        _validate_kv_cache_pool_plan(kv_cache_groups, pool_plan)
        return sum(region.size_bytes for region in pool_plan)
...
def get_kv_cache_config_from_groups(
    ...
    *,
    ...
    Args:
        ...
        available_memory: Memory available for KV cache in bytes
        pool_plan: Optional declarative physical-region plan.
        fixed_num_blocks: Exact profiling capacity, bypassing the config override.
...
def _max_memory_usage_bytes_from_groups(
    ...
    bytes_per_block = _pool_bytes_per_block(kv_cache_groups, pool_plan)
    total_blocks = 0
    for group in kv_cache_groups:
        spec = group.kv_cache_spec
        if isinstance(spec, UniformTypeKVCacheSpecs):
...
def _estimate_max_model_len_from_groups(
    ...
    def fits(model_len: int) -> bool:
        ...
        return (
            _max_memory_usage_bytes_from_groups(vllm_config, kv_cache_groups, pool_plan)
            <= available_memory
        )
...
def _auto_fit_max_model_len(
    ...
    pool_plans_per_worker: list[KVCachePoolPlan | None],
    ...
    Args:
        ...
        pool_plans_per_worker: Physical-region plans for the projected groups.
    ...
    for groups, pool_plan, avail_mem in zip(
        projected_groups_per_worker, pool_plans_per_worker, available_memory
    ):
        if not groups:
            continue
        worker_max = _estimate_max_model_len_from_groups(
            vllm_config, groups, avail_mem, pool_plan
        )
...
def get_kv_cache_configs(
    ...
    """Build cache configs through the resolved model and platform builder."""
    from vllm.v1.core.kv_cache_config_builder import build_kv_cache_configs

    return build_kv_cache_configs(vllm_config, kv_cache_specs, available_memory)


def _get_kv_cache_configs(
    vllm_config: VllmConfig,
    kv_cache_specs: list[dict[str, KVCacheSpec]],
    available_memory: list[int],
    plan_provider: KVCachePlanProvider | None = None,
    *,
    fixed_num_blocks: int | None = None,
) -> list[KVCacheConfig]:
    ...
    Args:
        ...
        available_memory: Memory available for KV cache in bytes for each
            worker.
        plan_provider: Resolved declarative plan provider, if any.
        fixed_num_blocks: Build an exact-capacity profiling config without
            changing ``num_gpu_blocks_override``.
    ...
    if plan_provider is None:
        from vllm.v1.core.kv_cache_plan import resolve_kv_cache_plan_provider

        plan_provider = resolve_kv_cache_plan_provider(vllm_config)
    ...
    if vllm_config.scheduler_config.disable_hybrid_kv_cache_manager:
        unify_hybrid_kv_cache_specs(merged_kv_cache_specs)

    # Get global KV cache groups after common spec normalization.
    declared_groups = (
        plan_provider.get_kv_cache_groups(
            vllm_config, MappingProxyType(merged_kv_cache_specs)
        )
    ...
    if declared_groups is None:
        ...
        validate_kv_cache_groups(merged_kv_cache_specs, declared_groups)
        global_kv_cache_groups = declared_groups
    ...
    pool_plans_per_worker = [
        plan_provider.get_kv_cache_regions(vllm_config, tuple(groups))
        if plan_provider is not None
        else None
    ...
    override = (
        fixed_num_blocks
        if fixed_num_blocks is not None
        else vllm_config.cache_config.num_gpu_blocks_override
    )
    if override is not None:
        ...
        for groups, pool_plan, avail_mem in zip(
            ...
            if not groups:
                ...
                continue
            bytes_per_block = _pool_bytes_per_block(groups, pool_plan)
            logger.info(
    ...
    check_memory = [
        avail_mem - _pool_bytes_per_block(groups, pool_plan) if groups else avail_mem
        for groups, pool_plan, avail_mem in zip(
            projected_groups_per_worker, pool_plans_per_worker, available_memory
        )
    ]
    ...
    if (
        fixed_num_blocks is None
        and vllm_config.model_config.original_max_model_len == -1
    ):
    ...
    if fixed_num_blocks is None:
        for groups, pool_plan, avail_mem in zip(
            projected_groups_per_worker, pool_plans_per_worker, check_memory
        ):
            if not groups:
                continue
            _check_enough_kv_cache_memory(
    ...
    for i, kv_cache_config in enumerate(kv_cache_configs):
        ...
        groups = kv_cache_config.kv_cache_groups
        pool_plan = pool_plans_per_worker[i]
        kv_cache_configs[i] = get_kv_cache_config_from_groups(
            vllm_config,
            groups,
            min_num_blocks * _pool_bytes_per_block(groups, pool_plan),
            pool_plan=pool_plan,
            fixed_num_blocks=min_num_blocks,
        )
