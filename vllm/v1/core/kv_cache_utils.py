...
def materialize_kv_cache_group_plan(
    kv_cache_specs: dict[str, KVCacheSpec], plan: KVCacheGroupPlan
) -> list[KVCacheGroupSpec]:
    """Validate and materialize a declarative group plan."""
    planned_names = [name for group in plan.groups for name, _ in group.layer_specs]
    ...
    for entry in plan.groups:
        if not entry.layer_specs:
            raise ValueError("KV cache group plan cannot contain an empty group")
        layer_specs = dict(entry.layer_specs)
        group_spec: KVCacheSpec
        if entry.use_uniform_type:
            uniform_spec = UniformTypeKVCacheSpecs.from_specs(layer_specs)
            if uniform_spec is None:
                raise ValueError("KV cache group plan specs cannot be aggregated")
            group_spec = uniform_spec
        else:
            group_spec = next(iter(layer_specs.values()))
            if any(spec != group_spec for spec in layer_specs.values()):
                raise ValueError("Non-aggregated KV cache group specs must be equal")
        groups.append(KVCacheGroupSpec(list(layer_specs), group_spec))
    return groups
...
def _pool_bytes_per_block(
    ...
    worker_plan: KVCacheWorkerPlan | None = None,
) -> int:
    ...
    """
    return _get_kv_cache_bytes_per_block(kv_cache_groups, worker_plan)
...
def _get_kv_cache_bytes_per_block(
    ...
    if worker_plan is not None:
        return worker_plan.bytes_per_block
...
def get_kv_cache_config_from_groups(
    ...
    Args:
        ...
        available_memory: Memory available for KV cache in bytes
        worker_plan: Optional executable accounting and placement plan.
    ...
    if worker_plan is not None:
        bytes_per_block = worker_plan.bytes_per_block
        num_blocks = may_override_num_blocks(
            vllm_config, available_memory // bytes_per_block
        )
        return worker_plan.materialize(vllm_config, num_blocks)
...
def get_kv_cache_groups(
    ...
    """
    _normalize_kv_cache_specs(vllm_config, kv_cache_spec)
...
def _normalize_kv_cache_specs(
    vllm_config: VllmConfig, kv_cache_specs: dict[str, KVCacheSpec]
) -> None:
...
def _max_memory_usage_bytes_from_groups(
    ...
    if worker_plan is not None:
        return worker_plan.max_memory_usage_bytes(vllm_config)
    ...
    def fits(model_len: int) -> bool:
        ...
        return (
            _max_memory_usage_bytes_from_groups(
                vllm_config, kv_cache_groups, worker_plan
            )
            <= available_memory
        )
...
def _auto_fit_max_model_len(
    ...
    worker_plans: list[KVCacheWorkerPlan | None],
    ...
    Args:
        ...
        worker_plans: Executable plans for the projected groups.
    ...
    for groups, worker_plan, avail_mem in zip(
        projected_groups_per_worker, worker_plans, available_memory
    ):
        if not groups:
            continue
        worker_max = _estimate_max_model_len_from_groups(
            vllm_config, groups, avail_mem, worker_plan
        )
...
def get_kv_cache_configs(
    ...
    return _get_kv_cache_configs(
        vllm_config, kv_cache_specs, available_memory, plan_provider
    )
...
def _get_kv_cache_configs(
    ...
    *,
    ...
    Args:
        ...
        available_memory: Memory available for KV cache in bytes for each
            worker.
        plan_provider: Resolved plan provider, if any.
        fixed_num_blocks: Exact block count used by profiling.
        check_capacity: Whether to run auto-fit and admission checks.
    ...
    _normalize_kv_cache_specs(vllm_config, merged_kv_cache_specs)
    group_plan = plan_provider.get_group_plan(
        vllm_config, MappingProxyType(merged_kv_cache_specs)
    )
    global_kv_cache_groups = (
        materialize_kv_cache_group_plan(merged_kv_cache_specs, group_plan)
        if group_plan is not None
        else get_kv_cache_groups(vllm_config, merged_kv_cache_specs)
    )
    ...
    worker_plans = [
        plan_provider.get_worker_plan(vllm_config, tuple(groups))
    ...
    if override is not None:
        ...
        for groups, worker_plan, avail_mem in zip(
            ...
            if not groups:
                ...
                continue
            bytes_per_block = _pool_bytes_per_block(groups, worker_plan)
    ...
    check_memory = [
        avail_mem - _pool_bytes_per_block(groups, worker_plan) if groups else avail_mem
        for groups, worker_plan, avail_mem in zip(
            projected_groups_per_worker, worker_plans, available_memory
        )
    ]

    if check_capacity and vllm_config.model_config.original_max_model_len == -1:
    ...
    for groups, worker_plan, avail_mem in zip(
        projected_groups_per_worker, worker_plans, check_memory
    ):
    ...
    for (
        ...
        if fixed_num_blocks is not None and projected_groups:
            if worker_plan is not None:
                config = worker_plan.materialize(vllm_config, fixed_num_blocks)
            else:
                saved_override = vllm_config.cache_config.num_gpu_blocks_override
                vllm_config.cache_config.num_gpu_blocks_override = fixed_num_blocks
                try:
                    config = get_kv_cache_config_from_groups(
                        vllm_config,
                        projected_groups,
    ...
    for i, kv_cache_config in enumerate(kv_cache_configs):
        ...
        groups = kv_cache_config.kv_cache_groups
        worker_plan = worker_plans[i]
        if worker_plan is not None:
            kv_cache_configs[i] = worker_plan.materialize(vllm_config, min_num_blocks)
        else:
