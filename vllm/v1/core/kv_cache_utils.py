...
def check_enough_kv_cache_memory(
    ...
    if kv_cache_spec:
        planner = get_kv_cache_planner(vllm_config)
        ...
        groups = planner.get_kv_cache_groups(dict(kv_cache_spec))
        check_memory = (
            available_memory - planner.get_kv_cache_bytes_per_block(groups)
            if groups
            else available_memory
        )
        policy_memory = planner.policy.get_max_memory_usage_bytes(groups)
        if policy_memory is None:
            get_needed_memory = partial(
                max_memory_usage_bytes, vllm_config, kv_cache_spec.values()
            )
            estimate_model_len = partial(
                estimate_max_model_len, vllm_config, kv_cache_spec
            )
        else:
            get_needed_memory = partial(planner.get_max_memory_usage_bytes, groups)
            estimate_model_len = partial(
...
class KVCachePlanningPolicy:
    """Model or platform differences used by the common KV cache planner."""
...
def resolve_kv_cache_planning_policy(
    vllm_config: VllmConfig,
...
class KVCachePlanner:
    """Common KV cache planning orchestration with injectable differences."""

    def __init__(
        self,
        vllm_config: VllmConfig,
        policy: KVCachePlanningPolicy | None = None,
    ) -> None:
        self.vllm_config = vllm_config
        self.policy = policy or resolve_kv_cache_planning_policy(vllm_config)
    ...
    def get_kv_cache_groups(
        ...
        if is_kv_cache_type_attention_free(kv_cache_spec):
            return []
        if groups := get_hisparse_kv_cache_groups(self.vllm_config, kv_cache_spec):
            return groups
        groups = self.policy.get_kv_cache_groups(kv_cache_spec)
        if groups is not None:
            return groups
        return _get_default_kv_cache_groups(self.vllm_config, kv_cache_spec)

    def get_kv_cache_bytes_per_block(
        self, kv_cache_groups: list[KVCacheGroupSpec]
    ) -> int:
        policy_bytes = self.policy.get_kv_cache_bytes_per_block(kv_cache_groups)
    ...
    def get_max_memory_usage_bytes(
        ...
        policy_bytes = self.policy.get_max_memory_usage_bytes(kv_cache_groups)
        if policy_bytes is not None:
            return policy_bytes
        return _default_max_memory_usage_bytes_from_groups(
            self.vllm_config, kv_cache_groups, self.get_kv_cache_bytes_per_block
        )
    ...
    def get_kv_cache_config_from_groups(
        ...
        if not kv_cache_groups:
            return KVCacheConfig(
                ...
                prefix_cache_retention_interval=(
                    self.vllm_config.cache_config.prefix_cache_retention_interval
                ),
            )

        if self.vllm_config.attention_config.hisparse_config is not None:
            host_budget = get_hisparse_host_pool_bytes(self.vllm_config)
        ...
        num_blocks = may_override_num_blocks(
            self.vllm_config, available_memory // bytes_per_block
        )
        kv_cache_tensors = self.policy.get_kv_cache_tensors(
            kv_cache_groups, num_blocks, bytes_per_block
        )
        if kv_cache_tensors is None:
            kv_cache_tensors = _get_default_kv_cache_tensors(
                self.vllm_config,
                kv_cache_groups,
                num_blocks,
                bytes_per_block,
    ...
    def plan(
        ...
        return self.get_kv_cache_config_from_groups(
            self.get_kv_cache_groups(kv_cache_spec), available_memory
        )


def get_kv_cache_planner(vllm_config: VllmConfig) -> KVCachePlanner:
    return KVCachePlanner(vllm_config)
...
def _get_default_kv_cache_tensors(
    vllm_config: VllmConfig,
    kv_cache_groups: list[KVCacheGroupSpec],
    num_blocks: int,
    bytes_per_block: int,
...
def get_kv_cache_config_from_groups(
    ...
    """Generate a KV cache config through the resolved planner."""
    return get_kv_cache_planner(vllm_config).get_kv_cache_config_from_groups(
...
def _get_default_kv_cache_groups(
    vllm_config: VllmConfig,
    kv_cache_spec: dict[str, KVCacheSpec],
) -> list[KVCacheGroupSpec]:
...
def get_kv_cache_groups(
    ...
    """Split model layers through the resolved KV cache planner."""
    return get_kv_cache_planner(vllm_config).get_kv_cache_groups(kv_cache_spec)
...
def _default_max_memory_usage_bytes_from_groups(
    vllm_config: VllmConfig,
    kv_cache_groups: list[KVCacheGroupSpec],
    get_bytes_per_block: Callable[[list[KVCacheGroupSpec]], int] = (
    ...
    bytes_per_block = get_bytes_per_block(kv_cache_groups)
    total_blocks = 0
    for group in kv_cache_groups:
        spec = group.kv_cache_spec
        if isinstance(spec, UniformTypeKVCacheSpecs):
...
def _max_memory_usage_bytes_from_groups(
    ...
    return get_kv_cache_planner(vllm_config).get_max_memory_usage_bytes(kv_cache_groups)
...
def _estimate_max_model_len_from_groups(
    ...
    kv_cache_groups: list[KVCacheGroupSpec],
    available_memory: int,
    planner: KVCachePlanner | None = None,
) -> int:
    ...
    planner = planner or get_kv_cache_planner(vllm_config)
    hisparse_enabled = (
    ...
    def fits(model_len: int) -> bool:
        ...
        if hisparse_enabled:
            try:
                config = planner.get_kv_cache_config_from_groups(
                    kv_cache_groups, available_memory
                )
            except ValueError:
                return False
            return get_max_concurrency_for_kv_cache_config(vllm_config, config) >= 1
        return planner.get_max_memory_usage_bytes(kv_cache_groups) <= available_memory
...
def _auto_fit_max_model_len(
    ...
    Args:
        ...
        available_memory: Memory available for KV cache in bytes for each
            worker.
        planner: Planner whose policy defines model or platform differences.
    ...
    for groups, avail_mem in zip(projected_groups_per_worker, available_memory):
        ...
        worker_max = _estimate_max_model_len_from_groups(
            vllm_config, groups, avail_mem, planner
        )
...
def get_kv_cache_configs(
    ...
    global_kv_cache_groups = planner.get_kv_cache_groups(merged_kv_cache_specs)
    ...
    if override is not None:
        ...
        for groups, avail_mem in zip(projected_groups_per_worker, available_memory):
            if not groups:
                ...
                continue
            bytes_per_block = planner.get_kv_cache_bytes_per_block(groups)
            logger.info(
    ...
    check_memory = [
        avail_mem - planner.get_kv_cache_bytes_per_block(groups)
        if groups
        else avail_mem
        for groups, avail_mem in zip(projected_groups_per_worker, available_memory)
    ]
    ...
    if vllm_config.model_config.original_max_model_len == -1:
        _auto_fit_max_model_len(
            vllm_config, projected_groups_per_worker, check_memory, planner
        )
    ...
    for groups, avail_mem in zip(projected_groups_per_worker, check_memory):
        ...
        _check_enough_kv_cache_memory(
            avail_mem,
            partial(planner.get_max_memory_usage_bytes, groups),
            vllm_config.model_config.max_model_len,
            partial(
                _estimate_max_model_len_from_groups,
                vllm_config,
    ...
    for projected_groups, kv_cache_spec_one_worker, available_memory_one_worker in zip(
        ...
        kv_cache_configs.append(
            planner.get_kv_cache_config_from_groups(
                projected_groups, available_memory_one_worker
            )
        )
    ...
    for i, kv_cache_config in enumerate(kv_cache_configs):
        ...
        groups = kv_cache_config.kv_cache_groups
        kv_cache_configs[i] = planner.get_kv_cache_config_from_groups(
            groups,
            min_num_blocks * planner.get_kv_cache_bytes_per_block(groups),
        )
