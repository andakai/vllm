...
def get_kv_cache_configs(
    ...
    """Build cache configs through the resolved model and platform planner."""
    from vllm.v1.core.kv_cache_plan import build_kv_cache_configs
...
def _get_kv_cache_configs(
    ...
    *,
    ...
    Args:
        ...
        available_memory: Memory available for KV cache in bytes for each
            worker.
        group_planner: Optional model-specific grouping function.
        region_planner: Optional model-specific physical-region function.
    ...
    declared_groups = (
        group_planner(vllm_config, MappingProxyType(merged_kv_cache_specs))
        if group_planner is not None
        else None
    )
    ...
    pool_plans_per_worker = [
        region_planner(vllm_config, tuple(groups))
        if region_planner is not None
        else None
