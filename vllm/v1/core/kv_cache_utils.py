...
def _validate_kv_cache_pool_plan(
    ...
    planned_layers = [layer for region in plan.regions for layer in region.layers]
...
def _get_kv_cache_bytes_per_block(
    ...
    if pool_plan is not None:
        ...
        return sum(region.size_bytes for region in pool_plan.regions)
...
def get_kv_cache_config_from_groups(
    ...
    if pool_plan is not None:
        ...
        region_offset = 0
        for region in pool_plan.regions:
...
def get_kv_cache_configs(
    ...
    pool_plans_per_worker = [
        plan_provider.get_pool_plan(vllm_config, tuple(groups))
        if plan_provider is not None
        else None
