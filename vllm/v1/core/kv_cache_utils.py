...
def check_enough_kv_cache_memory(
    ...
    """
    from vllm.v1.core.kv_cache_config_builder import resolve_kv_cache_config_builder

    resolve_kv_cache_config_builder(vllm_config).check_enough_kv_cache_memory(
...
def _build_default_kv_cache_tensors(
    vllm_config: VllmConfig,
    kv_cache_groups: list[KVCacheGroupSpec],
    num_blocks: int,
    size: int,
    ...
    """Build default tensor descriptors for a core-sized allocation."""
...
def get_kv_cache_config_from_groups(
    ...
    return resolve_kv_cache_config_builder(vllm_config).get_kv_cache_config_from_groups(
...
def get_kv_cache_groups(
    ...
    return resolve_kv_cache_config_builder(vllm_config).get_kv_cache_groups(
...
def _plan_kv_cache_configs(
    ...
    Args:
        builder: The resolved KV cache configuration builder.
        vllm_config: The global VllmConfig
    ...
    if override is not None:
        ...
        for groups, avail_mem in zip(projected_groups_per_worker, available_memory):
            if not groups:
                ...
                continue
            bytes_per_block = builder.get_pool_bytes_per_block(groups)
            logger.info(
    ...
    check_memory = [
        avail_mem - builder.get_pool_bytes_per_block(groups) if groups else avail_mem
        for groups, avail_mem in zip(projected_groups_per_worker, available_memory)
    ]

    if vllm_config.model_config.original_max_model_len == -1:
        builder._auto_fit_max_model_len(
    ...
    for groups, avail_mem in zip(projected_groups_per_worker, check_memory):
        ...
        _check_enough_kv_cache_memory(
            avail_mem,
            partial(builder.get_max_memory_usage_bytes, vllm_config, groups),
            vllm_config.model_config.max_model_len,
            partial(builder._estimate_max_model_len, vllm_config, groups),
        )
    ...
    for i, kv_cache_config in enumerate(kv_cache_configs):
        ...
        groups = kv_cache_config.kv_cache_groups
        kv_cache_configs[i] = builder.get_kv_cache_config_from_groups(
            vllm_config,
            groups,
            min_num_blocks * builder.get_pool_bytes_per_block(groups),
        )
...
def get_kv_cache_configs(
    ...
    return resolve_kv_cache_config_builder(vllm_config).get_kv_cache_configs(
