...
def get_kv_cache_config_from_groups(
    ...
    Args:
        ...
        available_memory: Memory available for KV cache in bytes
        builder: Builder supplying per-block sizing, when customized.
    ...
    bytes_per_block = (
        builder.get_kv_cache_bytes_per_block(kv_cache_groups)
        if builder is not None
        else _get_kv_cache_bytes_per_block(kv_cache_groups)
    )
...
def _max_memory_usage_bytes_from_groups(
    ...
    bytes_per_block = (
        builder.get_kv_cache_bytes_per_block(kv_cache_groups)
        if builder is not None
        else _pool_bytes_per_block(kv_cache_groups)
    )
    total_blocks = 0
    for group in kv_cache_groups:
        spec = group.kv_cache_spec
...
def _estimate_max_model_len_from_groups(
    ...
    def fits(model_len: int) -> bool:
        ...
        if hisparse_enabled:
            try:
                config = builder.get_kv_cache_config_from_groups(
        ...
        return (
            builder.max_memory_usage_bytes_from_groups(vllm_config, kv_cache_groups)
            <= available_memory
        )
...
def _auto_fit_max_model_len(
    ...
    Args:
        ...
        available_memory: Memory available for KV cache in bytes for each
            worker.
        builder: Builder used for model-specific planning hooks.
    ...
    for groups, avail_mem in zip(projected_groups_per_worker, available_memory):
        ...
        worker_max = _estimate_max_model_len_from_groups(
            vllm_config, groups, avail_mem, builder
        )
...
def get_kv_cache_configs(
    ...
    Args:
        ...
        available_memory: Memory available for KV cache in bytes for each
            worker.
        builder: Builder used for model- or platform-specific planning hooks.
    ...
    if override is not None:
        ...
        for groups, avail_mem in zip(projected_groups_per_worker, available_memory):
            if not groups:
                ...
                continue
            bytes_per_block = builder.get_kv_cache_bytes_per_block(groups)
            logger.info(
    ...
    check_memory = [
        avail_mem - builder.get_kv_cache_bytes_per_block(groups)
        if groups
        else avail_mem
        for groups, avail_mem in zip(projected_groups_per_worker, available_memory)
    ]
    ...
    if vllm_config.model_config.original_max_model_len == -1:
        _auto_fit_max_model_len(
            vllm_config, projected_groups_per_worker, check_memory, builder
        )
    ...
    for groups, avail_mem in zip(projected_groups_per_worker, check_memory):
        ...
        _check_enough_kv_cache_memory(
            avail_mem,
            partial(builder.max_memory_usage_bytes_from_groups, vllm_config, groups),
            vllm_config.model_config.max_model_len,
            partial(
    ...
    for i, kv_cache_config in enumerate(kv_cache_configs):
        ...
        kv_cache_configs[i] = builder.get_kv_cache_config_from_groups(
            vllm_config,
            groups,
            min_num_blocks * builder.get_kv_cache_bytes_per_block(groups),
        )
