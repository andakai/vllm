...
"""GLM-5.3-Flash KV cache grouping and shared tensor placement."""
...
def _get_glm5_kv_cache_groups(
    ...
    return (
        ...
        + create_kv_cache_group_specs(padded_specs, grouped_names)
    )


def _get_glm5_tensor_layout(
...
class Glm5NextKVCacheConfigBuilder(KVCacheConfigBuilder):
    """Share GLM Mamba/tail state with its MLA/indexer allocation."""
    ...
    def _get_custom_kv_cache_groups(
        ...
        return _get_glm5_kv_cache_groups(vllm_config, kv_cache_spec)
    ...
    def _build_kv_cache_tensors(
        ...
        layout = _get_glm5_tensor_layout(kv_cache_groups)
        if layout is None:
            return super()._build_kv_cache_tensors(
                vllm_config,
                kv_cache_groups,
                num_blocks,
                size,
    ...
    def _get_kv_cache_bytes_per_block(
        ...
        _, _, mla_names, idx_names, mla_page, idx_page, _ = layout
    ...
    def _get_max_memory_usage_bytes(
        ...
        layout = _get_glm5_tensor_layout(kv_cache_groups)
        if layout is None:
            return super()._get_max_memory_usage_bytes(vllm_config, kv_cache_groups)
        (
            attn_group,
            mamba_groups,
            mla_names,
