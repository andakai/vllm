...
def _get_glm5_groups(
    ...
    attn_group_spec = UniformTypeKVCacheSpecs.from_specs(attn_specs)
    assert attn_group_spec is not None
    groups = [KVCacheGroupSpec(list(attn_specs), attn_group_spec)]
    if tail_specs:
        idx_page = next(iter(idx_pages))
        padded_tail_specs: dict[str, KVCacheSpec] = dict(
            (name, replace(spec, page_size_padded=idx_page))
            for name, spec in tail_specs.items()
        )
        tail_group_spec = UniformTypeKVCacheSpecs.from_specs(padded_tail_specs)
        assert tail_group_spec is not None
        groups.append(KVCacheGroupSpec(list(padded_tail_specs), tail_group_spec))
    ...
    groups.extend(
        KVCacheGroupSpec(layer_names, padded_mamba) for layer_names in grouped_names
    )
    return groups
...
class Glm5NextKVCachePlanner:
    def __init__(self, delegate: KVCachePlanner) -> None:
        self.delegate = delegate

    def _get_kv_cache_groups(self, vllm_config, kv_cache_specs):
        if (
            vllm_config.scheduler_config.disable_hybrid_kv_cache_manager
            or vllm_config.attention_config.hisparse_config is not None
        ):
            return None
        return _get_glm5_groups(vllm_config, kv_cache_specs)

    def _get_kv_cache_regions(self, vllm_config, kv_cache_groups):
    ...
    def __call__(self, request: KVCachePlanningRequest):
        return self.delegate(
            request.with_declarative_plan(
                self._get_kv_cache_groups,
                self._get_kv_cache_regions,
            )
        )
