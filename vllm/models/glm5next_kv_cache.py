...
def _get_mamba_group_count(
    vllm_config: VllmConfig, mamba_names: list[str], mla_names: list[str]
) -> int | None:
...
def _get_groups(
    ...
    num_groups = _get_mamba_group_count(vllm_config, list(mamba_specs), mla_names)
...
def _get_bytes_per_block(
    vllm_config: VllmConfig, kv_cache_groups: list[KVCacheGroupSpec]
) -> int | None:
    del vllm_config
    if (layout := _get_layout(kv_cache_groups)) is None:
        return None
    return (
...
def _get_max_memory(
    ...
    total_blocks = attn_spec.max_memory_usage_pages(vllm_config)
    total_blocks += sum(
        cdiv(
    ...
    if layout.tail_names:
        total_blocks += 1
    bytes_per_block = _get_bytes_per_block(vllm_config, kv_cache_groups)
    assert bytes_per_block is not None
    return total_blocks * bytes_per_block


GLM5_NEXT_KV_CACHE_HOOKS = KVCachePlanningHooks(
    get_groups=_get_groups,
    get_bytes_per_block=_get_bytes_per_block,
